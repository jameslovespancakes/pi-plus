    ...
    try {
      return await rc.semaphore.run(
        async () => {
          ...
          rc.progress.agentStart(opts.phase, label, rowId, modelLabel);
...
/** "provider/id" for progress rows so every agent shows the model it routed to. */
function describeAgentModel(model: unknown): string | undefined {
  if (!model || typeof model !== "object") return undefined;
  const candidate = model as { provider?: unknown; id?: unknown };
  if (typeof candidate.id !== "string") return undefined;
  return typeof candidate.provider === "string" ? `${candidate.provider}/${candidate.id}` : candidate.id;
}
