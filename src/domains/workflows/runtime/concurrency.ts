...
/** Global agent semaphore. A null capacity is unlimited. */
export class Semaphore {
  ...
  constructor(max: number | null) {
    if (max !== null && (!Number.isInteger(max) || max < 1)) {
      throw new RangeError("Semaphore capacity must be a positive integer or null");
    }
    this.max = max;
  }
  ...
  async run<T>(fn: () => Promise<T>, options: { onQueueWaitMs?: (durationMs: number) => void; signal?: AbortSignal } = {}): Promise<T> {
    ...
    if (this.max !== null && this.active >= this.max) {
    ...
    try {
      ...
      // Transfer the reservation directly to the next waiter.
...
async function runParallel<T, Result>(
  ...
  throwIfAborted(options.signal);
  const results = Array.from<Result>({ length: thunks.length });
