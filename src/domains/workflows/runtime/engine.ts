...
export interface WorkflowContextOptions {
  ...
  /** Optional eager-submission cap for `parallel`. */
...
export async function runResolvedWorkflow(
  ...
  const durableRun = new DurableWorkflowRun(
    ...
    (error) => {
      ...
      progress.log(`run persistence failed: ${unknownErrorMessage(error)}`);
    },
  );
  ...
  const workflowOutcome = await captureOutcome(async () => {
    ...
    return perf.time("workflow.total_ms", () =>
      runWorkflowWithContext(rc, progress, mod, args, {
        ...
        submissionLimit: resolvedOptions.parallelSubmissionLimit ?? (resolvedOptions.concurrency === null ? undefined : resolvedOptions.concurrency * 2),
