...
/**
 * Run records and replay journals live outside the project by default so they
 * never clutter a working directory. Override with PI_WORKFLOW_RUNS_DIR, or set
 * it to "project" to restore the legacy `<cwd>/.pi/.workflow-runs` layout.
 */
export function workflowRunsDir(cwd: string): string {
  const configured = process.env.PI_WORKFLOW_RUNS_DIR?.trim();
  if (configured === "project") return join(cwd, WORKFLOW_RUNS_DIR);
  const root = configured && configured.length > 0
    ? configured
    : join(process.env.PI_AGENT_DIR ?? join(homedir(), ".pi", "agent"), "workflow-runs");
  const slug = basename(cwd).replace(/[^A-Za-z0-9._-]+/g, "-").slice(0, 40) || "project";
  ...
  try {
    const behavior = {
      ...
      opts: {
        ...
        candidatePatch: opts.candidatePatch
          ? {
              baselineOid: opts.candidatePatch.baselineOid,
              hash: createHash("sha256").update(opts.candidatePatch.patch).digest("hex"),
            }
          : undefined,
        worktreeBaseline: worktreeBaseline
          ? {
