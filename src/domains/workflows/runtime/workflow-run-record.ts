...
function isPersistedWorkflowRunOptions(value: unknown): value is PersistedWorkflowRunOptions {
  ...
  if (!isFiniteNumber(value.agentRetries)) return false;
  if (value.concurrency !== null && !isFiniteNumber(value.concurrency)) return false;
  if (value.maxAgents !== null && !isFiniteNumber(value.maxAgents)) return false;
  if (value.agentTimeoutMs !== null && !isFiniteNumber(value.agentTimeoutMs)) return false;
