...
import { validateWorkflowRunId, workflowRunsDir } from "./journal.ts";
...
export class ProjectWorkflowRunStore implements WorkflowRunStore {
  ...
  async list(): Promise<WorkflowRunRecord[]> {
    const dir = workflowRunsDir(this.cwd);
    let files: string[];
    try {
...
export function workflowRunRecordPath(cwd: string, runId: string): string {
  return join(workflowRunsDir(cwd), `${validateWorkflowRunId(runId)}${WORKFLOW_RUN_RECORD_SUFFIX}`);
}
